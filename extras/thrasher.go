	...
	for i := 0; i < 16; i++ {
		go func() {
			for {
				...
				if err := remote_put("http://138.197.65.174:3000/"+key, int64(len(value)), strings.NewReader(value)); err != nil {
					fmt.Println("PUT FAILED", err)
					resp <- false
					continue
				}

				ss, err := remote_get("http://138.197.65.174:3000/" + key)
				if err != nil || ss != value {
				...
				if err := remote_delete("http://138.197.65.174:3000/" + key); err != nil {
